...
foreach ($file_list as $path) {     // 画像ファイルのリストをループで処理
    ...
    if ($resize_w > 0) {
        // 横サイズ指定でリサイズ（縦は比率で自動計算）
        $hiritu = $resize_w / $original_w;
        $henkan_h = round($original_h * $hiritu);
        $im->resizeImage($resize_w, $henkan_h, Imagick::FILTER_LANCZOS, 1);
    } elseif ($resize_h > 0) {
        // 縦サイズ指定でリサイズ（横は比率で自動計算）
        $hiritu = $resize_h / $original_h;
        $henkan_w = round($original_w * $hiritu);
        $im->resizeImage($henkan_w, $resize_h, Imagick::FILTER_LANCZOS, 1);
    }
