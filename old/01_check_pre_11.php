    ...
    public static function checkProfiles(string $filepath): array {
        ...
        try {
            ...
            try {
                ...
                if ($exifProfile !== false) {
                    $exifInfo['has_profile'] = true;
                    $exifInfo['has_payload'] = (strlen($exifProfile) > 0);
                }

                if (function_exists('exif_read_data')) {
                    $exifArr = @exif_read_data($filepath, 'ANY', true, false);
                    $exifInfo['has_tags'] = (is_array($exifArr) && count($exifArr) > 0);
                }
            } catch (\ImagickException $e) {}
        ...
        if ($exifInfo['has_tags'] || $exifInfo['has_real']) {
            $profiles[] = 'EXIF:データあり';
        } elseif ($exifInfo['has_profile'] || $exifInfo['has_payload'] || $exifInfo['has_app1']) {
            $profiles[] = 'EXIF:入れ物のみ';
        }
