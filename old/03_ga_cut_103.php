...
if ($t_dh = opendir($title_dir)) {
	...
	while (($wa_folder = readdir($t_dh)) !== false) {							//タイトルフォルダ（180ガール）内
		...
		if ($wa_folder != "." && $wa_folder != "..") {				
			...
			$this_folder = $wa_folder_list[$wa_folder_num];			//画像ファイルの前付
...
//			echo $dest;																		https://www.php.net/manual/ja/imagick.cropimage.php
		...
		$seikei_renban_num = sprintf('%03d', $renban_counter); 	// 出力画像名の0埋め

//		$output_dest = $output_wa_dir . "/image-out" . "_" . $seikei_ga_file_num . "_" . $seikei_cut_num . "_" . $seikei_renban_num . ".jpg";
		$output_dest = $output_wa_dir . "/$this_folder" . "_" . $seikei_renban_num . ".jpg";				//フォルダ内画像を連番出力
...
//				$output_nokori_dest = $output_wa_dir . "/image-out" . "_" . $seikei_ga_file_num + 1 . "_" . $seikei_cut_num . "_" . $seikei_renban_num . ".jpg";
				$output_nokori_dest = $output_wa_dir . "/$this_folder" . "_" . $seikei_renban_num . ".jpg";			//フォルダ内画像を連番出力
