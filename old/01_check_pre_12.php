...
class MetaInspectorV3 {
    // App1 + IFD0 check to avoid empty Exif shells
        ...
        try {
            ...
            foreach ($profileLabels as $key => $label) {
                try {
                    ...
                    if ($data !== false) {
                        $profiles[] = $label . ':' . (strlen($data) > 0 ? 'データあり' : '入れ物のみ');
                    }
...
try {
    ...
    $exifInfo['has_profile'] = ($exifProfile !== false && strlen($exifProfile) > 0);
...
if (function_exists('exif_read_data') && preg_match('/\.jpe?g$/i', $filepath)) {
    ...
    // IFD0にタグがあるか（殻だけかどうか）をさらに確認
    $why = MetaInspectorV3::hasRealExifIFD0($filepath);
    $exifInfo['ifd0_tags'] = $why['ifd0_tags'] ?? 0;
...
if ($exifInfo['has_tags'] || $exifInfo['has_real']) {
    // 実際にタグが入っている
    $profiles[] = 'EXIF:データあり';
} elseif ($exifInfo['has_profile'] || $exifInfo['has_app1']) {
    // APP1 セグメントや空の profile だけ存在
    $profiles[] = 'EXIF:入れ物のみ';
}
