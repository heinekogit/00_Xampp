...
// ★ここに追加
$found_profiles = MetaInspectorV3::checkProfiles($file_list[$file_num]);
if (!empty($found_profiles)) {
    echo "　[メタ情報あり: " . implode(", ", $found_profiles) . "]";
    $log_content .= "num " . $file_num . "：" . $file . "　メタ情報あり: " . implode(", ", $found_profiles) . "\r\n";
}
...
//	ファイルに出力
//	https://www.php.net/manual/ja/function.file-put-contents.php} catch (ImagickException $e) {
                // プロファイルが無い場合は何もしない
            }
        }
        $im->destroy();
...
// ===== MetaInspectorV3: Single source for metadata checks =====
if (!class_exists('MetaInspectorV3')) {
class MetaInspectorV3 {
    // Minimal binary APP1 scan with IFD0 entry count to avoid "empty Exif shell" false positives
    public static function hasRealExifIFD0(string $path): array {
        $reason = ['app1'=>false, 'ifd0_tags'=>0, 'exif_read_count'=>0];
        $fh = @fopen($path, 'rb'); if(!$fh) return $reason;
        if (fread($fh,2)!="\xFF\xD8") { fclose($fh); return $reason; }
        while (!feof($fh)) {
            $b=fread($fh,1); if($b===false) break;
            if($b!="\xFF") continue;
            do{$m=fread($fh,1);}while($m==="\xFF");
            if($m===false)break;
            $mk=ord($m); if($mk===0xDA||$mk===0xD9)break;
            $lenBin=fread($fh,2); if(strlen($lenBin)<2)break;
            $len=unpack('n',$lenBin)[1]; if($len<2)break;
            ...
            if($mk===0xE1 && substr($payload,0,6)==="Exif\0\0"){
                ...
                if(strlen($tiff)>=8){
                    ...
                    $off=$le?unpack('V',substr($tiff,4,4))[1]:unpack('N',substr($tiff,4,4))[1];
                    if($off+2<=strlen($tiff)){
                        $cntBin=substr($tiff,$off,2);
                        $cnt=$le?unpack('v',$cntBin)[1]:unpack('n',$cntBin)[1];
        ...
        if (function_exists('exif_read_data')) {
            ...
            if (is_array($arr)) $reason['exif_read_count']=count($arr);
        }
        return $reason;
    }

    // Check ICC/XMP/IPTC presence; Exif only if it has real tags (not only APP1 shell)
    public static function checkProfiles(string $filepath): array {
        $profiles = [];
        try {
            $im = new \Imagick($filepath);
            foreach (['iptc','xmp','icc'] as $p) {
                try {
                    $data = $im->getImageProfile($p);
                    if ($data !== false && strlen($data) > 0) $profiles[] = $p;
                } catch (\ImagickException $e) {}
            }
            $hasExifProfile = false;
            try {
                $exifProfile = $im->getImageProfile('exif');
                $hasExifProfile = ($exifProfile !== false && strlen($exifProfile) > 0);
            } catch (\ImagickException $e) {}
            $hasExifTags = false;
            if (function_exists('exif_read_data')) {
                $arr = @exif_read_data($filepath, 'ANY', true, false);
                $hasExifTags = (is_array($arr) && count($arr) > 0);
            }
            $why = self::hasRealExifIFD0($filepath);
            $hasReal = ($why['ifd0_tags'] > 0);
            if ($hasExifProfile && ($hasExifTags || $hasReal)) {
                $profiles[] = 'exif';
            }
            $im->destroy();
        } catch (\Exception $e) {}
    ...
    // Build display label like "[メタ情報なし]" or "[メタ情報あり: exif/xmp/... ]"
    public static function labelFor(string $filepath): string {
        $p = self::checkProfiles($filepath);
        return empty($p) ? "[メタ情報なし]" : "[メタ情報あり: ".implode('/', $p)."]";
    }
}
}
