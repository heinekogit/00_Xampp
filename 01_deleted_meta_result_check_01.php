...
//	処理フォルダ　=================================================================================
	...
	$home_dir = "C:/xampp/htdocs/working/01_画像メタ情報_削除結果/";		
//	$out_dir = "C:/xampp/htdocs/working/02_output_gousei/";		
	
    if ($h_dh = opendir($home_dir)) {

//	ログファイルの用意	----------------------------------------------------------------
		$filename = 'C:/xampp/htdocs/working/01_画像メタ情報_削除結果/checklog_01.txt';
...
//	------------------------------------------------------------------------------------
        ...
        while (($title_folder = readdir($h_dh)) !== false) {
            if ($title_folder != "." && $title_folder != "..") {
                ...
                if (!is_dir($title_dir)) continue; // フォルダ以外はスキップ
        ...
        while (($wa_folder = readdir($t_dh)) !== false) {
            if ($wa_folder != "." && $wa_folder != "..") {
                $was_dir = $title_dir . $wa_folder;
                if (!is_dir($was_dir)) continue; // フォルダ以外はスキップ
